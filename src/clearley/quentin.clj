...
(defn impl-body [{:keys [name args body]}]
  `(~name ~(vec args) ~(deref body)))

; Used for lazily generating code snippets (allows interreferences).
; Provides an autogenerated autoincrementing :name and wraps the body
; in a delay. Stashed into the memo key ::parse-fns.
(defn stash-method [name tag args body-builder & key]
  (let [body (delay (apply body-builder key))]
    ; TODO save body
    (dotify
      (:name
        (generate ::parse-fns key
                  (fn [_ i] {:body body
                             :name (symbol (str name "_" i))
                             :tag tag
                             :args args}))))))
...
(defn gen-parser-protocols []
  ...
  `(do
     ...
     (deftype ~'Parser [~(apply symbol '(^int ^:unsynchronized-mutable goto))
...
(defn gen-parser-body [item-set initial? shift?]
  (let [initial-arglist (if (or initial? shift?) [] ['v0])
        body1 (if-let [a-const-reduce (const-reduce item-set)]
               ; Some parsers are trivial. If so, we short-circuit.
               (if shift?
                 `(let [~@(stream-binder)
                        ~'v0 (.current ~'stream)]
    ...
    ; TODO
    (if (<= (code-size body1) 160)
      (stash-method "parse"
                    java.lang.Object
                    (apply vector (apply symbol '(^ParseState state))
                           (if-not (or shift? initial?) initial-arglist))
                    (fn [& _] body1)
...
(defn parse-fn [grammar goal myns mem opts]
  (with-memoizer mem
    (binding [*myns* myns
      ...
      (let [r @(ns-resolve *myns* (item-parser-sym (build-item-sets goal grammar)
                                                   true false))
            ppcode (gen-parser-protocols)]
        (println "===")
        (binding [*print-meta* true]
